...
class Generator(nn.Module):
    ...
    def forward(self, picture, landmarks_real, landmarks_wanted): #img = x
        ...
        f2 = self.fc2(maxout)
        rsh = f2.reshape((x.size()[0], int(64), 10, 10))
...
tb = SummaryWriter(log_dir = '64_run')  
epoch = LAST_EPOCH
while(True):
    print("## EPOCH", epoch,"##")
    for step, batch in enumerate(train_dataloader):
        ...
        L_final = 10 * pixelwise_160_loss + 0.1 * pixelwise_80_loss + (1e-4)*pixelwise_40_loss + 0.1 * (adv_D1_loss + adv_D2_loss) + 0.02 * (identity_loss) + 1e-4 *(total_variation)
        ...
        tb.add_scalar( "Adversarial_D1/Train" , adv_D1_loss.data.cpu().numpy() ,   epoch*len(train_dataloader) + step)
        tb.add_scalar( "Adversarial_D2/Train" , adv_D2_loss.data.cpu().numpy() ,   epoch*len(train_dataloader) + step)
    ...
    if (epoch % 2 == 0):
        save_model(G, "G","D:models_64M",  epoch)
        save_model(D1,"D1",  "D:models_64M", epoch)
        save_model(D2,  "D2", "D:models_64M", epoch)
        save_optimizer(optimizer_G,G, "G", "D:optimizer_64M",epoch)
        save_optimizer(optimizer_D1,D1, "D1",  "D:optimizer_64M",epoch)
        save_optimizer(optimizer_D2,D2, "D2", "D:optimizer_64M",epoch)
